...
func _ready() -> void:
	...
	for level_path in DirAccess.get_files_at(DIR_PATH):
		if (regex.search(level_path)):
			...
			levelIconInstance.find_child("LevelIcon").texture = load(level_metadata.get("level_icon", "res://assets/sprites/default_texture.png"))
			levelIconInstance.find_child("Rank").texture = load(level_metadata.get("rank_icon", "res://assets/sprites/default_texture.png"))
